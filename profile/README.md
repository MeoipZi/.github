<h1 align="center">IHEI WORKSTATION TEAM MeoipZi</h1>

<p align="center">
  <img src="https://github.com/MeoipZi/.github/assets/96698362/09441582-f2b9-4bbb-b36c-8b2e10a4586a" alt="hi" width="500">
</p>

## 🙋‍♀️ Introduction
# Vintage clothing promotion website to enhance accessibility and convenience of information search 
In the fashion industry, there are various services, such as a platform that integrates and shows several online shopping malls and a site where you can share reviews on products. 
However, consumers find it difficult to obtain information because there are not many such services in the used clothing industry, and the credibility of shopping malls' products decreases due to problems such as lack of reviews. 
A website has been developed to solve this problem and allow consumers to conveniently access shopping malls specializing in second-hand clothing. 
It provides a connection link to several shopping malls that sell vintage clothing at once, and creates a networking site that allows site users to share information and share opinions with each other.
Site users can post their own outfit ideas that use vintage clothings and provide the place and price of the clothes. 
It is also possible to leave likes and reviews on specific shopping malls and products. 
Other users who see this can directly visit the shopping mall through the link, which can promote the consumption of second-hand clothing.
In addition, looking at outfit ideas using vintage clothing, second-handed products are no longer considered as rustic, and can be reborn as items that satisfy users' fashion needs while thinking about the Earth's environment.

## 🚀 Features
### 👩‍💻회원가입/로그인
 #### 🌈 Demo video
 [![Watch the video](https://img.youtube.com/vi/gO2GJ4Eess0/0.jpg)](https://www.youtube.com/watch?v=gO2GJ4Eess0&list=PLSsE3BpeXsN6XB9J7hjOMVBOR9-uuCDhz&index=3)

닉네임, 비밀번호, 비밀번호 확인하기를 입력해서 회원가입할수 있다

아이디와 비밀번호를 입력해서 로그인할 수 있다.



### 👩‍💻홈버튼<br>
 #### 🌈 Demo video
[![Watch the video](https://img.youtube.com/vi/oWYrOEObL2c/0.jpg)](https://www.youtube.com/watch?v=oWYrOEObL2c&list=PLSsE3BpeXsN6XB9J7hjOMVBOR9-uuCDhz&index=4)

**Vintage News**

MeoipZi에서 선별한 특별한 빈티지 소식을 알려준다.

**Partners**

제휴 쇼핑몰을 보여준다.
제휴 쇼핑몰 이미지를 클릭하면 해당 쇼핑몰 사이트로 바로 이동할 수 있다.

**Style**

MeoipZi에서 코디한 다양한 장르의 아웃핏을 제공한다. 

코디 사진을 누르면 코디 세부 페이지로 넘어간다.



### 👩‍💻검색<br>
 #### 🌈 Demo video
[![Watch the video](https://img.youtube.com/vi/yWJU6eyTK60/0.jpg)](https://www.youtube.com/watch?v=yWJU6eyTK60&list=PLSsE3BpeXsN6XB9J7hjOMVBOR9-uuCDhz&index=6)

MeoipZi 관리자가 상품과 코디 정보를 등록하고 관리한다.

카테고리 검색, 브랜드 검색, 장르별 검색, 총 3가지 기능을 제공한다.  

카테고리 종류에는 [상의], [하의], [원피스], [아우터]가 있다.

브랜드 종류에는 [나이키], [타미진스], [구찌], [챔피온], [칼하트], [A.P.C], [입생로랑], [라코스테], [폴로랄프로렌], [타미힐피거], [지고트], [M’S GRACY], [엠포리오], [VTZ], [어널로이드], [비바스튜디오], [자라]가 있다.

장르 종류에는 [Y2K], [클래식], [캐주얼], [스포티], [스트릿], [페미닌], [아메카지], [비즈니스], [빈티지] 가 있다.

사용자가 검색하려는 카테고리 혹은 검색하려는 브랜드를 누르면 그에 해당하는 상품 사진들을 모아볼 수 있다.

사용자가 상품을 누르면 상품 사진, 상품 판매처, 상품 판매처 사이트, 상품 이름, 상품 가격에 대한 정보를 볼 수 있다. 해당 상품의 링크를 타인과 공유하거나 상품을 북마크 할 수 있다.

사용자가 검색하려는 장르를 누르면 그에 해당하는 코디 사진들을 모아볼 수 있다.

사용자가 코디사진을 누르면 코디 사진, 코디 사진에 대한 설명, 해당 코디을 구성하는 상품 목록들에 대한 정보를 확인할 수 있다. 코디 좋아요, 북마크, 공유를 할 수 있고 댓글을 작성할 수 있다.




### 👩‍💻숏폼<br>
 #### 🌈 Demo video
[![Watch the video](https://img.youtube.com/vi/92LpetcADjw/0.jpg)](https://www.youtube.com/watch?v=92LpetcADjw&list=PLSsE3BpeXsN6XB9J7hjOMVBOR9-uuCDhz)

서비스 사용자는 자신의 숏폼을 업로드하고 MeoipZi에서 제작한 숏폼이나 다른 사용자가 제작한 숏폼을 인스타 피드 형식으로 확인할 수 있다.

숏폼을 누르면 숏폼에 대한 상세한 설명을 확인할 수 있고  숏폼 좋아요를 할 수 있으며 댓글을 작성할 수 있다.



### 👩‍💻커뮤니티 게시판<br>
 #### 🌈 Demo video
[![Watch the video](https://img.youtube.com/vi/qZmVoI5BJg8/0.jpg)](https://www.youtube.com/watch?v=qZmVoI5BJg8&list=PLSsE3BpeXsN6XB9J7hjOMVBOR9-uuCDhz&index=5)

브랜드/업체 탭: MeoipZi에서는 브랜드 관련 정보를 제공하거나 파트너 업체가 제공하는 공식 정보를 공지한다.

쇼핑&패션 탭: 서비스 사용자는 쇼핑과 패션에 관한 글을 게시할 수 있다.

자유게시판 탭:  서비스 사용자는 다른 서비스 이용자와 자유롭게 소통하고 교류할 수 있다.




### 👩‍💻마이페이지<br>
 #### 🌈 Demo video
[![Watch the video](https://img.youtube.com/vi/Rzu7hTUQNNg/0.jpg)](https://www.youtube.com/watch?v=Rzu7hTUQNNg&list=PLSsE3BpeXsN6XB9J7hjOMVBOR9-uuCDhz&index=2)

프로필 편집 : 사용자는 자신이 원하는 이미지와 닉네임 , 신장, 몸무게를 프로필 편집에서 설정할 수 있다.  사용자는 자신의 신장과 몸무게는 비공개로 설정할 수 있다.

"Like": 서비스 이용자가 좋아요한 [코디], [숏폼]을 모아볼 수 있다. 더보기 버튼을 통해 더 많은 항목을 확인할 수 있다.

"Post": [피드 탭], [스크랩 탭], [댓글 탭]으로 이루어져 있다.

  피드 탭 : 서비스 이용자는 자신이 업로드한 숏폼, 게시글 목록을 확인할 수 있다. 더보기 버튼을 통해 더 많은 항목을 확인할 수 있다.

  스크랩 탭: 서비스 이용자는 자신이 스크랩한 아웃핏, 상품 목록을 확인할 수 있다. 더보기 버튼을 통해 더 많은 항목을 확인할 수 있다.

  댓글 탭: 서비스 이용자는 자신이 댓글을 작성한 아웃핏, 숏폼, 게시판 게시글 목록을 확인할 수 있다. 더보기 버튼을 통해 더 많은 항목을 확인할 수 있다.


## 🛠️ Architecture
<img width="593" alt="dontworry drawio (1)" src="https://github.com/MeoipZi/.github/assets/83911440/b9d90ccf-9328-4ac4-b398-f1b97402f215">

## 👩🏻‍💻  Team MeoipZi

|[이윤진](https://github.com/yunjin-21)|[이규민](https://github.com/keke5149)|[신수정](https://github.com/chock-cho)|[이민경](https://github.com/Minlee01)|[최민주](https://github.com/hmuri)|
| :-: | :-: | :-: | :-: | :-: |
|  <img src="https://avatars.githubusercontent.com/yunjin-21" width="150"> | <img src="https://avatars.githubusercontent.com/keke5149" width="150">|  <img src="https://avatars.githubusercontent.com/chock-cho" width="150"> |   <img src="https://avatars.githubusercontent.com/Minlee01" width="150">| <img src="https://avatars.githubusercontent.com/hmuri" width="150">|
|Team Leader, Backend Developer|Backend Developer|Backend Developer|Frontend Developer|Frontend Developer|


#### Planning Period
2023.10 ~ 2024.01
#### Development Period
2024.02 ~ 2024-06 
<!--

**Here are some ideas to get you started:**

🙋‍♀️ A short introduction - what is your organization all about?
🌈 Contribution guidelines - how can the community get involved?
👩‍💻 Useful resources - where can the community find your docs? Is there anything else the community should know?
🍿 Fun facts - what does your team eat for breakfast?
🧙 Remember, you can do mighty things with the power of [Markdown](https://docs.github.com/github/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax)
-->
